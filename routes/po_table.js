...
router.post("/po_createTable", authorize(), function (req, res) {
    var slotArray = [];
    for (var i = 1; i <= 9; i++) {
        slotArray.push(i);
    }
...
router.post("/po_editTable", authorize(), function (req, res) {
    var tableId = req.body.tableId;
...
router.get("/po_deleteTable", authorize(Role.Admin), function (req, res) {
    var tableId = req.query.tableId;
    Po_Table.remove({ _id: tableId }, function (err, table) {
        if (err) {
            res.json({ success: false, msg: "error", data: "" });
        } else {
            res.json({ success: true, msg: "edited", data: "edited" });
