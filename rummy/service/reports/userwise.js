    ...
    if (transType && transType.toLowerCase() === "all") {
        ...
        if (userId && userId.length > 5) {
            ...
            if (user.role === UserRole.DISTRIBUTOR) {
                console.log("TransType-All ==> Player-ALL ===> Role ==> Distributor")
                const agentIds = await Player.find({ distributorId: user.id.toString() }, { _id: 1 })
                const agentsArr = agentIds.map(agent => agent._id);
                const userList = await Player.find({ agentId: { $in: agentsArr } }, { _id: 1 });
                ...
                const userListTR = await Player.find({ role: { $in: [UserRole.USER, UserRole.ADMIN_USER] }, agentId: { $in: agentsArr } }, { _id: 1 })
                ...
                console.log("TransType-All ==> Player-ALL ===> Role ==> AGENT")
                const userList = await Player.find({ agentId: user.id }, { _id: 1 })
                const userArr = userList.map(userl => userl._id);
                ...
                console.log("TransType-All ==> Player-ALL ===> Role . ==> Admin")
                const userListTR = await Player.find({ role: { $in: [UserRole.USER, UserRole.ADMIN_USER] } }, { _id: 1 })
                const userArrTR = userListTR.map(user => user._id);
        ...
        if ([TransactionType.BOOT, TransactionType.CHAL, TransactionType.WIN].includes(transType)) {

            if (userId && userId.length > 5) {
                console.log("TransType-[Boot,Chal,Win] ==> Player-Selected")
                conditions = {
                    ...conditions,
                    transType: transType,
                    userId: mongoose.Types.ObjectId(userId)
                ...
                if (user.role === UserRole.DISTRIBUTOR) {
                    console.log("TransType-[Boot,Chal,Win] ==> Player-All ===> Role ==> DISTRIBUTOR")
                    const agentIds = await Player.find({ distributorId: user.id.toString() }, { _id: 1 })
                    const agentsArr = agentIds.map(agent => agent._id);
                    const userList = await Player.find({ agentId: { $in: agentsArr } }, { _id: 1 });
                    const userArr = userList.map(userl => userl._id);
                    ...
                    console.log("TransType-[Boot,Chal,Win] ==> Player-All ===> Role ==> AGENT")
                    const userList = await Player.find({ agentId: user.id }, { _id: 1 })
                    const userArr = userList.map(userl => userl._id);
                    conditions = {
                        ...conditions,
                    ...
                    console.log("TransType-[Boot,Chal,Win] ==> Player-All ===> Role ==> Admin")
                    conditions = {
                        ...conditions,
                        transType: transType
                    }
        ...
        if ([TransactionType.GIFT, TransactionType.TIP].includes(transType)) {
            if (userId && userId.length > 5) {
                console.log("TransType-[Gift, TIP] ==> Player-Selectd")
                conditions = {
                    ...conditions,
                    transType: transType,
                    senderId: mongoose.Types.ObjectId(userId)
                ...
                if (user.role === UserRole.DISTRIBUTOR) {
                    console.log("TransType-[Gift, TIP] ==> Player-All ===> Role ==> Admin")
                    const agentIds = await Player.find({ distributorId: user.id.toString() }, { _id: 1 })
                    const agentsArr = agentIds.map(agent => agent._id);
                    const userList = await Player.find({ agentId: { $in: agentsArr } }, { _id: 1 });
                    const userArr = userList.map(userl => userl._id);
                    ...
                    console.log("TransType-[Gift, TIP] ==> Player-All ===> Role ==> AGENT")
                    const userList = await Player.find({ agentId: user.id }, { _id: 1 })
                    const userArr = userList.map(userl => userl._id);
                    conditions = {
                        ...conditions,
        ...
        if ([TransactionType.RECHARGE, TransactionType.RECHARGE_REVERT, TransactionType.SETTLEMENT].includes(transType)) {
            ...
            const userList = await Player.find({ role: { $in: [UserRole.USER, UserRole.ADMIN_USER] } }, { _id: 1 })
            const userArr = userList.map(userl => userl._id);
            let lookup;
            if (TransactionType.RECHARGE === transType) {

                if (userId && userId.length > 5) {
                    console.log("TransType-[RECHARGE] ==> Player-Selected")
                    conditionsTR = {
                        ...conditionsTR,
                        transType: transType,
                        receiverId: mongoose.Types.ObjectId(userId)
                    ...
                    if (user.role === UserRole.DISTRIBUTOR) {
                        console.log("TransType-[RECHARGE] ==> Player-All ===> Role ==> Distributor")
                        const agentIds = await Player.find({ distributorId: user.id.toString() }, { _id: 1 })
                        const agentsArr = agentIds.map(agent => agent._id);
                        ...
                        console.log("TransType-[RECHARGE] ==> Player-All ===> Role ==> Agent")
                        const userList = await Player.find({ agentId: user.id }, { _id: 1 })
                        const userArr = userList.map(userl => userl._id);
                        // const userListTR = await Player.find({ role: { $in: [UserRole.USER, UserRole.ADMIN_USER], agentId: { $in: agentsArr } } }, { _id: 1 })
                        // const userArrTR = userListTR.map(user => user._id);
                        ...
                        console.log("TransType-[RECHARGE] ==> Player-All ===> Role ==> Admin")
                        const userListTR = await Player.find({ role: { $in: [UserRole.USER, UserRole.ADMIN_USER] } }, { _id: 1 })
                ...
                if (userId && userId.length > 5) {
                    console.log("TransType-[RECHARGE_REVERT, SETTLEMENT] ==> Player-Slected")
                    conditionsTR = {
                        ...conditionsTR,
                        transType: transType,
                        senderId: mongoose.Types.ObjectId(userId)
                    ...
                    if (user.role === UserRole.DISTRIBUTOR) {
                        console.log("TransType-[RECHARGE_REVERT, SETTLEMENT] ==> Player-All ===> Role ==> Distributor")
                        const agentIds = await Player.find({ distributorId: user.id.toString() }, { _id: 1 })
                        const agentsArr = agentIds.map(agent => agent._id);
                        ...
                        console.log("TransType-[RECHARGE_REVERT, SETTLEMENT] ==> Player-All ===> Role ==> AGENT")
                        const userList = await Player.find({ agentId: user.id }, { _id: 1 })
                        const userArr = userList.map(userl => userl._id);
                        ...
                        console.log("TransType-[RECHARGE_REVERT, SETTLEMENT] ==> Player-All ===> Role ==> Admin")
                        const userListTR = await Player.find({ role: { $in: [UserRole.USER, UserRole.ADMIN_USER] } }, { _id: 1 })
                        const userArrTR = userListTR.map(user => user._id);
                        conditionsTR = {
                            ...conditionsTR,
