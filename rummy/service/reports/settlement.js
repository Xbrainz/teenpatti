        ...
        if ([TransactionType.SETTLEMENT, TransactionType.RECHARGE_REVERT].includes(transType)) { //For Transaction Type Recharge Revert and Settlement
            if (user.role === UserRole.DISTRIBUTOR) {
                if (reportType === ReportType.AGENT) {
                    ...
                    console.log("TransType-Settlement,RechargeRevert ==>  Role-Distributor ==> reportType-Player....")
                    const agentIds = await Player.find({ role: UserRole.AGENT, distributorId: user.id }, { _id: 1 })
                    const agentsArr = agentIds.map(agent => agent._id);
                ...
                if (reportType === ReportType.USER) {
                    console.log("TransType-Settlement,RechargeRevert ==>  Role-Agent ==> reportType-Player....")
                    const userIds = await Player.find({ role: { $in: [UserRole.ADMIN_USER, UserRole.USER] }, agentId: user._id }, { _id: 1 })
                    const usersArr = userIds.map(userl => userl._id);
                    conditions = {
                        ...conditions,
                        senderId: { $in: usersArr }
                ...
                if (reportType === ReportType.DISTRIBUTOR) {
                    ...
                    const distributorIds = await Player.find({ role: UserRole.DISTRIBUTOR }, { _id: 1 })
                    ...
                    const agentIds = await Player.find({ role: UserRole.AGENT }, { _id: 1 })
                    const agentsArr = agentIds.map(agent => agent._id);
                    conditions = {
                        ...conditions,
                        senderId: { $in: agentsArr }
                    ...
                    console.log("TransType-Settlement,RechargeRevert ==>  Role-Admin ==> reportType-Player....")
                    const userIds = await Player.find({ role: { $in: [UserRole.ADMIN_USER, UserRole.USER] } }, { _id: 1 })
                    const usersArr = userIds.map(userl => userl._id);
                    conditions = {
                        ...conditions,
                        senderId: { $in: usersArr }
            ...
            if (user.role === UserRole.DISTRIBUTOR) {
                if (reportType === ReportType.AGENT) {
                    ...
                    console.log("TransType-Recharge ==> Role-Distributor ==> reportType-Player....")
                    const agentIds = await Player.find({ role: UserRole.AGENT, distributorId: user.id }, { _id: 1 })
                    const agentsArr = agentIds.map(agent => agent._id);
                    console.log(agentsArr);
                    const userIds = await Player.find({ role: { $in: [UserRole.ADMIN_USER, UserRole.USER] }, agentId: { $in: agentsArr } }, { _id: 1 })
                ...
                if (reportType === ReportType.USER) {
                    console.log("TransType-Recharge ==> Role-Agent ==> reportType-Player....")
                    const userIds = await Player.find({ role: { $in: [UserRole.ADMIN_USER, UserRole.USER] }, agentId: user._id }, { _id: 1 })
                    const usersArr = userIds.map(userl => userl._id);
                    conditions = {
                        ...conditions,
                ...
                if (reportType === ReportType.DISTRIBUTOR) {
                    ...
                    console.log("TransType-Recharge ==> Role-Admin ==> reportType-Player....")
                    const userIds = await Player.find({ role: { $in: [UserRole.ADMIN_USER, UserRole.USER] } }, { _id: 1 })
                    const usersArr = userIds.map(userl => userl._id);
                    conditions = {
                        ...conditions,
