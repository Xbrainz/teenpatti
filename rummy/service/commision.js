...
async function calculateCommission(tableId, playerId) {
    ...
    let agent = await User.findOne({ _id: user.agentId });
    let distributor = await User.findOne({ _id: agent.distributorId });
    let totalCommissionoriginal = 0;

    console.log("table.players[playerId].=========",table.players[playerId]);
