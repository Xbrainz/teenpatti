...
function isTrail(cardSet) {
	...
	return cardSet[0].rank === cardSet[1].rank && cardSet[2].rank === cardSet[0].rank && cardSet[2].rank === cardSet[3].rank;
}

function isPureSeq(cardSet) {
    let sorted = _.sortBy(cardSet, 'priority');
    let sortedRank = _.sortBy(cardSet, 'rank');
    return ((sorted[0].priority + 1 === sorted[1].priority && sorted[1].priority + 1 === sorted[2].priority && sorted[2].priority + 1 === sorted[3].priority) || (sortedRank[0].rank + 1 === sortedRank[1].rank && sortedRank[1].rank + 1 === sortedRank[2].rank && sortedRank[2].rank + 1 === sortedRank[3].rank)) && (sorted[0].type === sorted[1].type && sorted[1].type === sorted[2].type && sorted[2].type === sorted[3].type);
}

function isSeq(cardSet) {
    let sorted = _.sortBy(cardSet, 'priority');
    let sortedRank = _.sortBy(cardSet, 'rank');
    return (sorted[0].priority + 1 === sorted[1].priority && sorted[1].priority + 1 === sorted[2].priority && sorted[2].priority + 1 === sorted[3].priority) || (sortedRank[0].rank + 1 === sortedRank[1].rank && sortedRank[1].rank + 1 === sortedRank[2].rank && sortedRank[2].rank + 1 === sortedRank[3].rank);
}

function isColor(cardSet) {
    return (cardSet[0].type === cardSet[1].type && cardSet[1].type === cardSet[2].type && cardSet[2].type === cardSet[3].type);
}

function isPair(cardSet) {
    return (cardSet[0].rank === cardSet[1].rank || cardSet[1].rank === cardSet[2].rank || cardSet[2].rank === cardSet[3].rank || cardSet[0].rank === cardSet[3].rank);
}
        ...
        if (set1D[1].priority > set2D[1].priority) {
            ...
            if (set1D[2].priority > set2D[2].priority) {
                ...
                return false;
                 ...
                 if (set1D[3].priority > set2D[3].priority) {
					return true;
				} else if (set1D[3].priority < set2D[3].priority) {
					return false;
				}
				else {
					return _options.wininingPriority.cardType[set1D[3].type].priority >
                    _options.wininingPriority.cardType[set2D[3].type].priority;
				}
            }
        }
    }
...
function getPairRank(set) {
    ...
    } else if (set[2].priority === set[3].priority) {
        return set[2].priority;
    }else if (set[0].priority === set[3].priority) {
        return set[0].priority;
    }
    return -1;
}
