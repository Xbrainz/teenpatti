...
let newPlayer = getLastActivePlayer(id, players, avialbleSlots, table.maxPlayers);
		...
		if(!newPlayer.cardSeen)
		{
			...
			let newPlayer22 = getNextActivePlayer(id, players, avialbleSlots, table.maxPlayers);
	...
	{
		players = await getNextSlotForTurn(id, players, avialbleSlots, table.maxPlayers, table._id);
...
async function setNextPlayerTurn(players, avialbleSlots, tableId) {
	let activeTurnPlayer = getActionTurnPlayer(players);
	let maxPlayers = 9;
	let playersaa = await getNextSlotForTurn(activeTurnPlayer.id, players, avialbleSlots, maxPlayers, tableId)
	return playersaa;
}
